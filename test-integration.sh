...
test_dns_verify() {
    ...
    assert_output_contains "Verify with no provider shows error" "No provider configured" dokku dns:verify || true
...
test_dns_app_management() {
    ...
    assert_output_contains "App-specific report works" "Domain Status Table" dokku dns:report "$TEST_APP"
    
    # Test sync (will show AWS CLI not configured, which is expected)
    assert_output_contains "Sync shows expected message" "AWS CLI" dokku dns:sync "$TEST_APP"
